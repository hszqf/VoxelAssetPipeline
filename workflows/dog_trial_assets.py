...
CELL_RESOLUTION = 64
ASSETS = ["dog_golden"]
SOURCE_REFERENCE = "examples/dog_trial/reference_dog_icon_three_view_clean.png"


def make_dog_golden() -> VoxelModel:
    m = VoxelModel("dog_golden", 24, 15, 14)

    # Selected candidate 6 rebuilt from a three-view reference: one continuous
    # flat-backed body with no raised saddle or back patch.
    m.box(5, 16, 4, 9, 4, 11, "sand")
    m.box(6, 15, 3, 5, 5, 10, "sand_light")

    m.box(15, 20, 8, 13, 4, 10, "sand")
    m.box(19, 23, 7, 10, 5, 9, "cream")
...
def generate() -> list[dict]:
    ...
    manifest: list[dict] = []
    render_reference_sheet(models[0], OUT_DIR / "reference_dog_icon_three_view_clean.png", include_icon=True)
    render_reference_sheet(models[0], OUT_DIR / "reference_dog_three_view_clean.png", include_icon=False)
        ...
        manifest.append(
            {
                ...
                "observed": "Rebuilt from clean three-view reference: golden puppy with flat back, floppy ears, cream muzzle, and short tail.",
            }
        )
