...
2. Prefer a single source sheet with `Icon + Front 3/4 + Side + Front + Top` in one image, then stop for human approval of style, direction, and scale.
...
5. Review in `viewer/index.html`; the Reference pane shows the source sheet first, then generated `Icon / Front 3/4 / Side / Front / Top`.
...
`generate-dog-trial` demonstrates the source-sheet flow with one approved image containing `Icon + Front 3/4 + Side + Front + Top`.
