...
2. Default to one approved source sheet containing `Icon + Side + Front + Top` in a single image. Use multi-angle sheets only for exploration.
...
5. Render per-asset review views: `Source`, `Icon`, `Side`, `Front`, and `Top`.
...
- If orientation is ambiguous, generate or request a source sheet with `Icon + Side + Front + Top` before `.vox` work.
