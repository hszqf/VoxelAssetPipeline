...
3. Default to one approved AI source sheet for exactly one asset, containing `Front 3/4 design + Back 3/4 design + registered Side 64-grid + registered Front 64-grid + registered Top 64-grid` in a single image. The Side/Front/Top orthographic design views must already include visible 64x64 cell guides, a bounding cell frame, and shared coordinate registration.
4. Require orthographic registration in the source sheet: Side maps length X vs height Y, Front maps width/depth Z vs height Y, and Top maps length X vs width/depth Z. The views must share front/head direction, ground baseline, body center, height, length, width, and major feature positions such as legs, head, tail, ears, horns, wings, handles, or markings.
5. After generation, estimate the visible bounding box from the Side, Front, and Top 64-grid views and write a short self-check report with target bounds, observed bounds, tolerance, registration checks, and pass/fail.
6. Reject the source sheet before asking for approval if the orthographic design views do not show the 64-cell guide, if the asset ignores the confirmed scale contract, if the asset fills the whole 64x64 frame without intent, if the Side/Front/Top views are not in the same scale system, or if the orthographic registration fails.
7. Do not silently regenerate. If the source sheet fails the bbox or registration self-check, tell the user the failed measurements/checks first, then regenerate only under the confirmed scale contract or ask for a revised scale contract. Failed sheets may be shown as failure evidence, but they are not approval candidates.
8. Stop after the first valid source sheet and wait for user confirmation before creating voxel geometry, `.vox`, manifests, or viewer data.
9. After confirmation, create voxel models at the agreed game-cell scale. Default: one game cell is `64 x 64 x 64`; assets should occupy only their declared proportion inside that cell.
10. After voxel geometry exists, render deterministic per-asset review views from the same `VoxelModel`: `Source`, `Icon`, `Front 3/4`, `Side`, `Front`, and `Top`. Every generated source sheet, multi-row reference sheet, and projection review image must include the asset's model/manifest name in the upper-left corner of its row.
11. Run structural validation. Every model must pass `single_connected_component`; `floating_component_sizes` must be empty. Do not rely only on visual inspection.
12. Update the static viewer data and ask the user to inspect assets one by one.
13. Only after user approval, run the project adapter to integrate assets into the game.
...
- Estimate and report Side/Front/Top bounding boxes and registration checks after generation. Do not ask for source approval until this report passes.
